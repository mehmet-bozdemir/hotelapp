<template>
    <form @submit.prevent="update">
        <div class="grid grid-cols-6 gap-4">
            ...
            <div class="col-span-4">
                <label class="block font-medium mb-1 text-gray-500">Street</label>
                <input v-model="form.street" type="text" class="w-full p-2 rounded-md shadow-sm border border-gray-300"/>
                <div class="input-error" v-if="form.errors.street">{{form.errors.street}}</div>
            </div>

            <div class="col-span-2">
                <label class="label">Street Nr</label>
                <input v-model.number="form.street_nr" type="text" class="input"/>
                <div class="input-error" v-if="form.errors.street_nr">{{form.errors.street_nr}}</div>
            </div>

            <div class="col-span-6">
                <label class="label">Price</label>
                <input v-model.number="form.price" type="text" class="input"/>
                <div class="input-error" v-if="form.errors.price">{{form.errors.price}}</div>
            </div>
